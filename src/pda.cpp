...
#ifdef _OPENMP
#pragma omp parallel for num_threads (ncore) if (n >= OMP_MIN_VALUE) private(i) \
  firstprivate(mean_v, mean_w, sd_v, A, b, t0, T0)
#endif
    for (i = 0; i < n; i++) {
...
//' @export
// [[Rcpp::export]]
arma::mat rplba1_test(unsigned int n, arma::vec A, arma::vec b, double t0, arma::vec mean_v,
  arma::vec mean_w, arma::vec sd_v, double rD, double swt, bool debug = false) {
...
arma::mat rplba2(int n, arma::vec A, arma::vec b, double t0, arma::vec mean_v,
                 ...
                 double swt, int ncore = 1, bool debug = false) {
...
#ifdef _OPENMP
#pragma omp parallel for num_threads (ncore) if (n >= OMP_MIN_VALUE) \
private(i) firstprivate(mean_v, mean_w, sd_v, sd_w, A, b, t0)
#endif
  for(i = 0; i < n; i++)
  {  // Stage 1 LBA
    int R_tmp;
...
//' @export
// [[Rcpp::export]]
arma::mat rplba2_test(unsigned int n, arma::vec A, arma::vec b, double t0, arma::vec mean_v,
                 arma::vec mean_w, arma::vec sd_v, arma::vec sd_w, double rD,
                 double swt, int ncore = 1, bool debug = false) {
  // This is non-parallel version
  arma::vec x0(n), x1(n), v0(n), v1(n), dt0(n), dt1(n);
...
arma::vec n1PDF_plba1(arma::vec x, arma::vec A, arma::vec b, arma::vec mean_v,
                      ...
                      double rD, double swt, int n, double h, int ncore,
                      bool debug) {
