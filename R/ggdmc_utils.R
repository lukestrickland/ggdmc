...
density.dmc <- function(x, C=NA, ...) {
  ...
  if ( !any(is.na(C)) ) {
    ...
    if (all(no.dns)) stop("There are no densities to plot!")
    dns1 <- dns[!no.dns]
...
#' @export
getdqp <- function(sim, facs, probs, bw = "nrd0") {
                             ...
                             bw="nrd0", report=10, save.dat=FALSE)
{
  ...
  colnames(thetas) <- samples$p.names

  ## random select n.post samples from thetas for each para
  posts <- thetas[sample(c(1:nrow(thetas)), n.post, replace = F), ]

  ## Construct a NA container with same dimension as data to store simulated data
  ## Should change to data.table
  sim <- data.frame(matrix(nrow = n.post*n.rep, ncol = ncol(DT)))
  names(sim) <- names(DT)

  for (i in names(DT)) {
    if (!is.null(levels(samples$data[[i]])))
  ...
  if (save.dat) {
    ...
    sim.dqp <- getdqp(sim, facs, probs)
    dat.dqp <- getdqp(sim=samples$data, facs, probs)  ## replace sim with samples$data
    x1 <- x3 <- NULL
...
##'
##' @param x a prior distribution list, usually created by
##' \code{BuildPrior}
